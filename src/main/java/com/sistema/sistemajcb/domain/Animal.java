package com.sistema.sistemajcb.domain;
...
public class Animal {
    ...
    private final UUID id; // Imutável após criação
    ...
    public Animal(String brincoRgd, LocalDate dataNascimento, Sexo sexo, UUID maeId, UUID loteInicial) {
        if (dataNascimento.isAfter(LocalDate.now())) {
            throw new IllegalArgumentException("O bezerro não pode nascer no futuro.");
        }
    ...
    public void registrarDesmame() {
        ...
        if (mesesIdade < 5) {
            throw new IllegalStateException("Animal muito jovem para desmame precoce (Mínimo 5 meses).");
        }
